...
if __name__ == "__main__":
    ...
    for p in ps:
        ...
        pto_max = 10
        d.pto = p.perturbation_order_total.data[:10]
        ds.append(d)
    ...
    plt.plot(4e7, 1e-3, 'o', color='white')
    plt.text(6e5, 1e-3, r'$\sim 1/\sqrt{N}$', ha='left', va='top')
    ...
    plt.legend(loc='best', fontsize=7, ncol=1)
    #plt.ylim([1e-4, 1e1])
    plt.xlim([1e-2, 1e9])
    plt.axis('equal')
    #plt.ylim([1e-5, 1e0])
    ...
    plt.bar(x, pto, alpha=0.75, color=color)
    #plt.semilogy([], [])
    #plt.ylim(top=1)
    plt.xlabel('Perturbation order')
    plt.ylabel('Probabilbity')
    plt.xticks(np.arange(0, 10), labels=[f'{i:d}' for i in range(0, 10)])
