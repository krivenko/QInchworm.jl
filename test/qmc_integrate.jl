    ...
    @testset "qmc_time_ordered_integral()" begin
        ...
        @testset "d = 1, constant integrand" begin
            let d = 1, c = contour, f = t -> 1.0, N = 2^17
                val, N_s = qmc_time_ordered_integral(f, d, c, c(0.1), c(0.5); τ=τ, N=N)
                @test isapprox(val, -0.4, rtol=1e-4)
                val, N_s = qmc_time_ordered_integral(f, d, c, c(0.1), c(2.0); τ=τ, N=N)
                @test isapprox(val, -0.9-1.0im, rtol=1e-4)
                val, N_s = qmc_time_ordered_integral(f, d, c, c(0.1), c(5.0); τ=τ, N=N)
                @test isapprox(val, -0.9-4.0im, rtol=1e-4)
                val, N_s = qmc_time_ordered_integral(f, d, c, c(0.1), c(6.5); τ=τ, N=N)
                @test isapprox(val, -0.4-5.0im, rtol=1e-4)
                val, N_s = qmc_time_ordered_integral(f, d, c, c(0.1), c(6.9); τ=τ, N=N)
    ...
    @testset "qmc_time_ordered_integral_n_samples()" begin
        ...
        @testset "d = 1, constant integrand" begin
            let d = 1, c = contour, f = t -> 1.0, N_samples = 2^16
                val, N = qmc_time_ordered_integral_n_samples(f, d, c, c(0.1), c(0.5);
                                                             τ=τ, N_samples=N_samples)
                @test isapprox(val, -0.4, rtol=1e-4)
                val, N = qmc_time_ordered_integral_n_samples(f, d, c, c(0.1), c(2.0);
                                                             τ=τ, N_samples=N_samples)
                @test isapprox(val, -0.9-1.0im, rtol=1e-4)
                val, N = qmc_time_ordered_integral_n_samples(f, d, c, c(0.1), c(5.0);
                                                             τ=τ, N_samples=N_samples)
                @test isapprox(val, -0.9-4.0im, rtol=1e-4)
                val, N = qmc_time_ordered_integral_n_samples(f, d, c, c(0.1), c(6.5);
                                                             τ=τ, N_samples=N_samples)
                @test isapprox(val, -0.4-5.0im, rtol=1e-4)
                val, N = qmc_time_ordered_integral_n_samples(f, d, c, c(0.1), c(6.9);
                                                             τ=τ, N_samples=N_samples)
                @test isapprox(val, -5.0im, rtol=1e-4)
            end
        end
    ...
    @testset "qmc_inchworm_integral_root()" begin
        ...
        @testset "d_before = 0, d_after = 3, constant integrand" begin
            let d_before = 0,
                ...
                val = qmc_inchworm_integral_root(f, d_before, d_after, c, t_i, t_w, t_f;
                                                 N=N)
